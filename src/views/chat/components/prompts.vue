<template>
  <div class="mt-17 h-full w-full flex flex-col px-20 lt-sm:mt-8 lt-sm:px-10">
    ...
    <div class="search mt-18 w-full lt-sm:mt-9">
      <n-input v-model:value="searchVal" placeholder="搜索" :size="largerThanSm ? 'large' : 'medium'"
        class="input b-rd-10 bg-#FFFFFFA6 text-#47484D" @input="handelSearch">
        <template #prefix>
    ...
    <n-spin :show="loading" stroke="#ffffff">
      <div class="flex-1 overflow-y-auto">
        ...
        <empty v-if="!filteredData.length && !loading" :type="activeIndex" />
      </div>
    </n-spin>
  </div>
</template>
